    ...
    for epoch in range(start_epoch+1, hparams.training_epochs):
        ...
        for (x_batch_val, _) in iterator(hparams, num_batches):
            ...
            if batch_num % 100 == 0:
                ...
                #utils.save_images(np.reshape(x_reconstr_mean_val, [-1, 28, 28]), [10, 10],'{}/reconstr_{:02d}_{:04d}.png'.format(hparams.sample_dir, epoch, batch_num))
                #utils.save_images(np.reshape(x_batch_val, [-1, 28, 28]), [10, 10], '{}/orig_{:02d}_{:04d}.png'.format(hparams.sample_dir, epoch, batch_num))
                #utils.save_images(np.reshape(x_sample_val, [-1, 28, 28]), [10, 10], '{}/sampled_{:02d}_{:04d}.png'.format(hparams.sample_dir, epoch, batch_num))
