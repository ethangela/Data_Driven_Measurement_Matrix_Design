...
def x_input(hparams, dim_like_, noise_var_):
    ...
    x_true = xs_dict[0].reshape(1,12288)
    ...
    matrixA_path = 'matrix_A_seed_{}_mea_dim_{}.npy'.format(seed_no, dim_like_)
    if not os.path.exists(matrixA_path):
        print('**** measurement matrix file does not exist... creating new one ****')
        A = np.random.randn(12288, dim_like_)
        A = A.astype('float32')
        np.save(matrixA_path, A)
    A = np.load(matrixA_path)
    
    return x_true, noise_mat3d, A
...
def compress_stats_main(hparams, mask, round_mcmc, n_sample, inpaint_size=8, used_indice_list=None, first_count=False, block_per_round=4, lambdas=0.9):
    ...
    dim_prior = z_dim #100
    dim_like = 64*64*3
    num_measurements = int(np.sum(mask))
    ...
    x_true, noise, A = x_input(hparams, num_measurements, noise_var)
...
def compress_mcmc_main(hparams, mask, round_mcmc=10, round_mcmc_start=0):
    ...
    N = 2000
    burn = int(0.5*N)
    num_measurements = int(np.sum(mask)) #!!!!!!!!!only changeable jun16, need to align with number of ones (in one dimension) in mask 
    noise_var = hparams.noise_std
    ...
    # print('original type:')
    # print(x_true.dtype)
    x_true, noise, A = x_input(hparams, num_measurements, noise_var) #!!!!!!!!!only changeable jun16
...
if __name__ == '__main__':
    ...
    #adaptive_mcmc
    ini_mask = np.zeros((64, 64, 3))
    ini_mask[1,1,:] = 1
    for i in range(12):
        if i == 0:
            compress_mcmc_main(HPARAMS, ini_mask, round_mcmc=1)
            new_mask, new_used_indice_list = compress_stats_main(HPARAMS, mask=ini_mask, round_mcmc=1, n_sample=100, inpaint_size=8, used_indice_list=None, first_count=True, block_per_round=4, lambdas=1.5)
        else:
            compress_mcmc_main(HPARAMS, new_mask, round_mcmc=1)
            new_mask, new_used_indice_list = compress_stats_main(HPARAMS, mask=new_mask, round_mcmc=1, n_sample=100, inpaint_size=8, used_indice_list=new_used_indice_list, first_count=False, block_per_round=4, lambdas=1.5)
