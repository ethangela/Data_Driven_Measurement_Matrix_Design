...
def get_image(image_path, image_size, is_crop=True, resize_w=64, is_grayscale = False):
    return transform(_imread(image_path, is_grayscale), image_size, is_crop, resize_w)
...
def _imread(path, is_grayscale = False):
    if (is_grayscale):
        return imread(path, flatten = True).astype(np.float32)
    else:
...
def center_crop(x, crop_h, crop_w=None, resize_w=64):
    if crop_w is None:
        crop_w = crop_h
    h, w = x.shape[:2]
    print('original image shape check: {}'.format(x.shape))
