...
def get_l2_loss_region(image1, image2): #aug9
    ...
    mask_ = np.load('../../uncertainty2/celeb_var_top_1_2_adhoc.npy')[64:192,64:192] #(128,128)
...
def get_pixl_l2_loss(image1, image2):
    assert image1.shape == image2.shape
    return np.mean((image1.reshape(3, 128, 128) - image2.reshape(3, 128, 128))**2, axis=0)  #(128,128)
...
def get_estimator(hparams, model_type):
    if model_type == 'map' and hparams.net == 'glow':
        ...
        if hparams.measurement_type == 'circulant':
            ...
            estimator = estimators.glow_annealed_langevin_estimator_adaptive(hparams)
...
def get_A(hparams): #jul28
    if hparams.measurement_type == 'gaussian':
        ...
        A = np.load(hparams.celeb_gaussian_A)
    elif hparams.measurement_type == 'gaussian_block':
        #A = hparams.variance_amount * np.random.randn(hparams.n_input, hparams.num_measurements) / np.sqrt(hparams.num_measurements) 
        A = np.load(hparams.celeb_extreme_A)
    elif hparams.measurement_type == 'gaussian_block_general':
        A = np.load(hparams.celeb_propotional_A) ###July6
    elif hparams.measurement_type == 'gaussian_block_adaptive': 
        if hparams.adaptive_round_count == 0:
            A = np.load(hparams.celeb_propotional_A) 
        else:
            ...
            A_path = os.path.join(npy_dir, 'variance_ADAPTIVE_measurement_{}_noise_{}_ROUND_{}.npy'.format(hparams.num_measurements, noise_info, int(hparams.adaptive_round_count-1)))
            A = np.load(A_path) 
    elif hparams.measurement_type == 'gaussian_block_data_driven':###Sep1
        A = np.load(hparams.celeb_block_data_driven_A)
    elif hparams.measurement_type == 'gaussian_data_driven':###Aug30
        if hparams.adaptive_round_count == 0:
            A = np.load(hparams.celeb_gaussian_A)
        else:
            noise_info = noise_info = '_'.join(str(hparams.noise_std).split('.'))
            npy_dir = '../../src/{}/data_driven/noise_{}/measurement_{}/npy'.format(hparams.load_img_no, noise_info, hparams.num_measurements)
...
# def partial_circulant_torch(inputs, filters, indices, sign_pattern):
#     n = np.prod(inputs.shape[1:])#batch_size,input
#     # vectorize stuff
#     bs = inputs.shape[0]
#     input_reshape = inputs.view(bs,n)

#     # multiply input with random bernoulli
#     input_sign = input_reshape * sign_pattern #batch_size,input

#     def to_complex(tensor):
#         zeros = torch.zeros_like(tensor) #b,n
#         concat = torch.cat((tensor, zeros), axis=0) #2b,n
#         reshape = concat.view(2,-1,n) #2,b,n
#         return reshape.permute(1,2,0) #b,n,2

#     # convert to two-dimensional complex value
#     complex_input = to_complex(input_sign) #batch_size, input, 2
#     complex_filter = to_complex(filters) #input, mea, 2
...
#     print('shape check', input_fft.size())
#     print('shape check', filter_fft.size())
